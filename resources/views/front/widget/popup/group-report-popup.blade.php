 <!-- create status -->
 <div class="hidden lg:p-20 uk- open" id="group-report-popup" uk-modal="">
   ...
   <div class="uk-modal-dialog tt relative overflow-hidden mx-auto bg-white shadow-xl rounded-lg md:w-[520px] w-full dark:bg-dark2">
       ...
       <div class="text-center py-4 border-b mb-0 dark:border-slate-700">
           <h2 class="text-sm font-medium text-black"> {{$name}}  </h2>
       ...
       <form class="needs-validation data-form" action="{{ route('group-report.store')  }} " method="POST" enctype="multipart/form-data">
         @csrf
        <div class="space-y-5 mt-3 p-2"> 
            <input type="hidden" name="group_id" value="{{$group->id}}">
            <div class="mb-4">
                <label for="reason" class="block text-gray-700 text-sm font-bold mb-2">Reason for Leaving</label>
                <select name="reason" id="reason" class="block shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" required>
