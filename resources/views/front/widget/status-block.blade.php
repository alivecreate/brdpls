 ...
 <div class="hidden lg:p-20 uk- open" id="create-status" uk-modal="">
   ...
   <div class="uk-modal-dialog tt relative overflow-hidden mx-auto bg-white shadow-xl rounded-lg md:w-[520px] w-full dark:bg-dark2">
       ...
       <div class="text-center py-4 border-b mb-0 dark:border-slate-700">
           <h2 class="text-sm font-medium text-black">555 Create Status </h2>
       ...
       <form class="needs-validation data-form" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <div class="flex items-center gap-2 text-sm py-2 px-4 font-medium flex-wrap">
            ...
            <label type="button" for="imageUploadInput" class="custom-button flex items-center gap-1.5 bg-sky-50 text-sky-600 rounded-full py-1 px-2 border-2 border-sky-100 dark:bg-sky-950 dark:border-sky-900">  
                <ion-icon name="image" class="text-base"></ion-icon>  
                <input type="file"  onchange="previewImage2(event)" 
                    name="gallery" id="imageUploadInput" class="hidden" accept="image/*">
                    Image 
            </label>

            <button type="submit" class="flex items-center gap-1.5 bg-teal-50 text-teal-600 rounded-full py-1 px-2 border-2 border-teal-100 dark:bg-teal-950 dark:border-teal-900">  
        ...
        <div class="p-5 flex justify-between items-center">
            <div>
                <button class="inline-flex items-center py-1 px-2.5 gap-1 font-medium text-sm rounded-full bg-slate-50 border-2 border-slate-100 group aria-expanded:bg-slate-100 aria-expanded: dark:text-white dark:bg-slate-700 dark:border-slate-600" type="button"> 
                    Everyone
                    <ion-icon name="chevron-down-outline" name="visibility" class="text-base duration-500 group-aria-expanded:rotate-180"></ion-icon> 
                </button>
